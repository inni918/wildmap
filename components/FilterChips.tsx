  ...
  return (
    <div className="bg-surface/95 backdrop-blur-sm border-b border-border">
      ...
      {chipsExpanded && (
        ...
        >
          {/* ── 分類 Chips（v1.0 只有露營，暫時隱藏；未來加入車宿/釣魚等分類時再開啟） ── */}
          {/* <ChipButton label="🏕 全部" selected={freeFilter === 'all'} onClick={() => onFreeFilterChange('all')} />
          <ChipButton label="🏕 付費露營地" selected={freeFilter === 'paid'} onClick={() => onFreeFilterChange(freeFilter === 'paid' ? 'all' : 'paid')} />
          <ChipButton label="🏕 免費露營地" selected={freeFilter === 'free'} onClick={() => onFreeFilterChange(freeFilter === 'free' ? 'all' : 'free')} />
