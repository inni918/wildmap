-- ============================================================
-- Wildmap Migration Step 1: DROP 舊表 + CREATE 新表
-- 請在 Supabase Dashboard → SQL Editor 執行
-- 執行前請確認 /tmp/wildmap-backup/ 備份完整
-- ============================================================

-- ============================================================
-- STEP 1: DROP 所有舊表（依照依賴順序，子表先 DROP）
-- ============================================================

-- 先 DROP views
DROP VIEW IF EXISTS public.user_spot_signals CASCADE;

-- DROP 子表（有 FK 的先 DROP）
DROP TABLE IF EXISTS public.user_events_daily_summary CASCADE;
DROP TABLE IF EXISTS public.user_events CASCADE;
DROP TABLE IF EXISTS public.user_profiles_cache CASCADE;
DROP TABLE IF EXISTS public.user_spot_signals CASCADE;
DROP TABLE IF EXISTS public.featured_achievements CASCADE;
DROP TABLE IF EXISTS public.user_category_stats CASCADE;
DROP TABLE IF EXISTS public.admin_audit_log CASCADE;
DROP TABLE IF EXISTS public.business_badges CASCADE;
...
DROP TABLE IF EXISTS public.spot_images CASCADE;
...
DROP TABLE IF EXISTS public.spot_edits CASCADE;
...
-- DROP 舊 ENUM types (如果有)
DROP TYPE IF EXISTS public.user_role CASCADE;
DROP TYPE IF EXISTS public.spot_category CASCADE;
DROP TYPE IF EXISTS public.spot_type CASCADE;
DROP TYPE IF EXISTS public.spot_status CASCADE;
DROP TYPE IF EXISTS public.spot_quality CASCADE;
DROP TYPE IF EXISTS public.feature_category CASCADE;
DROP TYPE IF EXISTS public.achievement_category CASCADE;
DROP TYPE IF EXISTS public.achievement_tier CASCADE;
...
-- DROP 舊 trigger function
DROP FUNCTION IF EXISTS public.handle_updated_at() CASCADE;

-- ============================================================
-- STEP 2: CREATE 新 Schema (schema-v1.sql 內容)
-- ============================================================
...
CREATE POLICY "spots_insert_auth" ON public.spots FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "spots_update_auth" ON public.spots FOR UPDATE USING (auth.uid() IS NOT NULL);

-- feature_definitions
CREATE POLICY "feature_definitions_select_all" ON public.feature_definitions FOR SELECT USING (true);
...
CREATE POLICY "feature_votes_update_own" ON public.feature_votes FOR UPDATE USING (auth.uid() = user_id);
...
-- spot_images
CREATE POLICY "spot_images_select_all" ON public.spot_images FOR SELECT USING (true);
CREATE POLICY "spot_images_insert_own" ON public.spot_images FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "spot_images_delete_own" ON public.spot_images FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "user_achievements_insert_auth" ON public.user_achievements FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
-- business_claims
CREATE POLICY "business_claims_select_own" ON public.business_claims FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "business_claims_insert_auth" ON public.business_claims FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE OR REPLACE TRIGGER set_updated_at_users BEFORE UPDATE ON public.users FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_spots BEFORE UPDATE ON public.spots FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_ratings BEFORE UPDATE ON public.ratings FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_comments BEFORE UPDATE ON public.comments FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_user_achievements BEFORE UPDATE ON public.user_achievements FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_user_stats BEFORE UPDATE ON public.user_stats FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_updated_at_business_claims BEFORE UPDATE ON public.business_claims FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

-- ============================================================
-- Done. Schema v1.0 ready.
-- 接下來請執行 migration-step2-seed-data.sql 匯入種子資料
-- ============================================================
